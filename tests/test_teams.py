...
from meharness.teams.backend_detect import BackendDetectionError, detect_backend, detect_pane_backend
from meharness.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from meharness.agents.tool_filter import (
...
class TestModels:
    ...
    def test_unique_team_name(self, tmp_dir):
        with patch("meharness.teams.models.Path.home", return_value=Path(tmp_dir)):
            name1 = unique_team_name("my-team")
            assert name1 == "my-team"
            (Path(tmp_dir) / ".meharness" / "teams" / "my-team").mkdir(parents=True)
...
class TestBackendDetect:
    ...
    def test_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("meharness.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_no_backend_raises(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("meharness.teams.backend_detect.shutil.which", return_value=None):
...
class TestCoordinatorMode:
    ...
    def test_enabled_with_flag_and_env(self):
        with patch.dict(os.environ, {"MEHARNESS_COORDINATOR_MODE": "1"}):
            assert is_coordinator_mode(enable_flag=True) is True

    def test_flag_without_env(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MEHARNESS_COORDINATOR_MODE", None)
    ...
    def test_match_session_mode_switch_to_coordinator(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert os.environ.get("MEHARNESS_COORDINATOR_MODE") == "1"
...
class TestConfigExtensions:
    def test_teammate_mode_defaults(self):
        from meharness.config import AppConfig
    ...
    def test_load_config_with_team_fields(self, tmp_dir):
        from meharness.config import load_config
    ...
    def test_invalid_teammate_mode(self, tmp_dir):
        from meharness.config import ConfigError, load_config
        config_path = Path(tmp_dir) / "config.yaml"
        config_path.write_text(
            "providers:\n"
            "  - name: test\n"
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from meharness.teams.transcript import load_transcript, save_transcript
    ...
    def test_load_nonexistent(self, tmp_dir):
        from meharness.teams.transcript import load_transcript
        with patch("meharness.teams.models.Path.home", return_value=Path(tmp_dir)):
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        # BASE_PERSONA/PLAN_MODE_INSTRUCTIONS 常量已在 prompts.py 重构为
        # PromptSection 后移除，改为校验当前身份段内容
        from meharness.prompts import build_system_prompt
        prompt = build_system_prompt()
        assert "You are Meharness" in prompt
    ...
    def test_coordinator_overrides_plan(self):
        # coordinator_mode 直接返回协调者提示词，不再拼接普通 agent 的 plan 指令
        from meharness.prompts import build_system_prompt
        prompt = build_system_prompt(coordinator_mode=True)
        assert "coordinator" in prompt.lower()
