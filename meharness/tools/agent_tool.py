...
class AgentTool(Tool):
    ...
    description = (
        ...
        "\n"
        "The Agent tool launches specialized agents (subprocesses) that autonomously handle complex tasks. Each agent "
        "type has specific capabilities and tools available to it. Use subagent_type to select a predefined agent type "
        "(e.g. Explore, Plan, general-purpose), or omit it to fork yourself — a fork inherits your full conversation "
        "context. Use team_name to spawn a long-running teammate in an existing team.\n"
        "\n"
        "When NOT to use the Agent tool:\n"
        "- If you want to read a specific file path, use the ReadFile tool or the Glob tool instead, to find the match "
        "more quickly\n"
        "- If you are searching for a specific class definition like \"class Foo\", use the Glob or Grep tool instead, "
        "to find the match more quickly\n"
        "- If you are searching for code within a specific file or set of 2-3 files, use the ReadFile tool instead of "
        "the Agent tool, to find the match more quickly\n"
        "- Other tasks that are not related to the agent types above\n"
        "\n"
        "Usage notes:\n"
        "- Always include a short description (3-5 words) summarizing what the agent will do\n"
        "- When the agent is done, it will return a single message back to you. The result returned by the agent is "
        "not visible to the user. To show the user the result, you should send a text message back to the user with a "
        "concise summary of the result.\n"
        "- You can optionally run agents in the background using the run_in_background parameter. When an agent runs "
        "in the background, you will be automatically notified when it completes — do NOT sleep, poll, or proactively "
        "check on its progress. Continue with other work or respond to the user instead.\n"
        "- Foreground vs background: Use foreground (default) when you need the agent's results before you can proceed "
        "— e.g., research agents whose findings inform your next steps. Use background when you have genuinely "
        "independent work to do in parallel.\n"
        "- To continue a previously spawned agent, use SendMessage with the agent's ID or name as the `to` field. The "
        "agent resumes with its full context preserved.\n"
        "- The agent's outputs should generally be trusted\n"
        "- Clearly tell the agent whether you expect it to write code or just to do research (search, file reads, web "
        "fetches, etc.), since it is not aware of the user's intent\n"
        "- If the user specifies that they want you to run agents \"in parallel\", you MUST send a single message with "
        "multiple Agent tool use content blocks.\n"
        "- You can optionally set `isolation: \"worktree\"` to run the agent in a temporary git worktree, giving it an "
        "isolated copy of the repository. The worktree is automatically cleaned up if the agent makes no changes; if "
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from meharness.agents.fork import ForkError, build_forked_messages
        ...
        if p.subagent_type is None:
            from meharness.context import clone_replacement_state
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from meharness.agents.fork import ForkError, build_forked_messages
        from meharness.agents.parser import AgentDef
        from meharness.agents.tool_filter import build_teammate_tools
        from meharness.agent import Agent as AgentClass
        from meharness.conversation import ConversationManager
        from meharness.permissions import (
            DangerousCommandDetector,
        ...
        from meharness.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        try:
            if backend == BackendType.TMUX:
                from meharness.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from meharness.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from meharness.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from meharness.client import create_client
