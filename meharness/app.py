...
from meharness.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from meharness.commands.completion import CompletionPopup
from meharness.commands.handlers import register_all_commands
from meharness.config import MCPServerConfig, ProviderConfig
from meharness.hooks import HookContext, HookEngine, load_hooks
...
from meharness.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from meharness.cache import FileCache
from meharness.tools import ToolRegistry, create_default_registry
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".meharness", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".meharness" / "history"
...
class MeharnessApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_MEHARNESS_THEME)
        self.theme = "meharness"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        # --- Worktree 系统初始化 ---
        from meharness.config import WorktreeConfig
    ...
    async def _show_plan_approval(self) -> None:
        from meharness.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from meharness.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from meharness.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from meharness.permission_dialog import InlinePermissionWidget
