...
class WorktreeConfig:
    symlink_directories: list[str] = field(default_factory=lambda: ["node_modules", ".venv", "vendor", "artifacts"])
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".meharness" / "config.yaml",
        cwd / ".meharness" / "config.yaml",
        cwd / ".meharness" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .meharness/config.yaml "
            "in project or ~/.meharness/config.yaml"
        )
    return merged
