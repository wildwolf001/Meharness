...
def _session_path(meharness_dir: Path) -> Path:
    return meharness_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(meharness_dir)
...
def load_worktree_session(meharness_dir: Path) -> WorktreeSession | None:
    path = _session_path(meharness_dir)
    if not path.exists():
        return None
    try:
