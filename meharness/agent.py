...
from meharness.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from meharness.conversation import ThinkingBlock as ConvThinkingBlock
...
from meharness.hooks import HookContext, HookEngine, ToolRejectedError
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".meharness" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".meharness" / "plans"
