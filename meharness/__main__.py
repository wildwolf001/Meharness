...
from meharness.hooks import HookConfigError, HookEngine, load_hooks
from meharness.permissions import PermissionMode


def main() -> None:
    # 先确保 .meharness/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".meharness").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".meharness/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="meharness", description="Meharness AI coding assistant")
    parser.add_argument(
        "--mode",
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    ...
    from meharness.client import create_client, resolve_context_window
    from meharness.conversation import ConversationManager
    from meharness.memory.instructions import load_instructions
    ...
    from meharness.worktree import WorktreeManager
